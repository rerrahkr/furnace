...
SCCIManager::SCCIManager() noexcept
    : hasLoadedLib_(false)
{
}
...
bool SCCIManager::initializeChips()
{
    ...
    for (auto i = 0; i < iCnt; ++i)
    {
        ...
        for (auto j = 0; j < siCnt; ++j)
        {
            SoundChip *sc = sif->getSoundChip(j);
            auto c = static_cast<SC_CHIP_TYPE>(sc->getSoundChipType());
...
bool SCCIManager::attach([[maybe_unused]] DivSystem sys, [[maybe_unused]] DivDispatch *dispatch)
{
#ifdef _WIN32
    switch (sys)
    {
...
bool SCCIManager::hasAttached([[maybe_unused]] DivDispatch *dispath) const
{
#ifdef _WIN32
    return connections_.count(dispath);
#else
    return false;
#endif
}

bool SCCIManager::detach([[maybe_unused]] DivDispatch *dispatch)
{
#ifdef _WIN32
...
bool SCCIManager::write([[maybe_unused]] DivDispatch *dispatch, [[maybe_unused]] addr_t addr, [[maybe_unused]] data_t data)
{
#ifdef _WIN32
    if (connections_.count(dispatch))
